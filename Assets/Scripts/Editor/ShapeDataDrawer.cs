...
public class ShapeDataDrawer : Editor
{
    ...
    private void DrawBoardTable()
    {
        ...
        for (var row = 0; row < ShapeDataInstance.rows; row++)
        {
            ...
            for (var columns = 0; columns < ShapeDataInstance.columns; columns++)
            {
                EditorGUILayout.BeginHorizontal(rowStyle);
                var data = EditorGUILayout.Toggle(ShapeDataInstance.board[row].column[columns], dataFieldStyle);
                ShapeDataInstance.board[row].column[columns] = data;
